...
\note{Instead of a Sanity number, characters just have permanent side effects (manias, phobias, ...) when stress goes above the limit. These can then be roleplayed and can also change over time. (e.g. "recovering in a mental ward" as a way to getting rid of them)}

\todoi{Add an intro explaining Stress. (and maybe rename all uses of "Sanity" by "Stress")}
...
Usually any Advantages or Drawbacks rolled will result in an instantaneousor short-term effect. Though if they're Shaken (see \autopageref{shaken}) then the effects will be more permanent or long-term.

\todoi{Be clearer about how to resolve Advantages and Drawbacks}
\todoi{Replace this table with some text instead.}
...
If your Stress goes above your limit you must make a Willpower check with 1 difficulty dice for each point you are above your limit.
...
If you fail this check you must choose either \textit{Fight}, \textit{Flight}, or \textit{Freeze}. You then remain in the state you chose and lose one Stress per minute until your Stress goes below twice your limit again.
...
\subsection{Becoming Shaken} \label{shaken}
...
On the other hand, if you get an Advantage on a Sanity check with a difficulty \textit{higher}
...
\subsection{Bonds} \label{sanity-bonds}
...
\subsection{Fainting}
Whenever you gain Stress you can also choose to faint and only gain one Stress instead of the full value. You remain unconsious for a number of minutes equal to the original Stress you would've taken.

\subsection{Trauma}
\todoi{Make rules for this!}

\subsection{Adapting to Horror}
Over time you can also adapt to horrors, this is represented by Hardened levels.

Hardened levels represent how used your character is to seeing different kinds of horrors.
There's 4 categories: \textit{Violence}, \textit{Helplessness}, \textit{Self}, and \textit{Unnatural}.
...
\ul{
    ...
    \item Maybe I should add some rules for permanent injuries... mostly just for RP reasons. Maybe something like: ">= 1/2 wound limit in one hit => permanent injury"
}
...
\ul{
    ...
    \ul{
        ...
        \item Nope, don't do it! -- tooboots said so
...
\ul{
    ...
    \item See if successes on 4 or nothing on 4 is best.
    I'm leaning towards nothing on 4 for simplicity and to make high difficulties less punishing, but it might make rolls with fewer dice less "eventful" sometimes by rolling more blanks.
    \item Should I make clear in the rules that advantages / drawbacks aren't solely the responsibility of the GM and players can (and should!) come up with the results of them too
}
}
